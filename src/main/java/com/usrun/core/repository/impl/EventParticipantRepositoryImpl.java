...
public class EventParticipantRepositoryImpl implements EventParticipantRepository {
  ...
  public List<UserEventDTO> getUserParticipant(long eventId, int offset, int count, String name) {
    ...
    try {
      List<UserEventDTO> users = namedParameterJdbcTemplate.query(
          "SELECT user.userId,displayName,province,avatar  FROM user LEFT JOIN eventParticipant"
  ...
  public List<TeamEventDTO> getTeamParticipant(long eventId, int offset, int count) {
    ...
    try {
      List<TeamEventDTO> teams = namedParameterJdbcTemplate.query(
          "SELECT team.teamId,teamName,province,thumbnail,totalMember  FROM team LEFT JOIN eventParticipant"
  ...
  public int getTotalTeamOfEvent(long eventId) {
    ...
    List<Integer> resp = namedParameterJdbcTemplate
        .query(sql, params, (rs, i) -> rs.getInt("number"));
    if (!resp.isEmpty()) {
      return resp.get(0);
    }
    return 0;
