  ...
  private OAuth2UserInfo verifyGoogle(String token) throws GeneralSecurityException, IOException {
    ...
    GoogleIdTokenVerifier verifier =
        new GoogleIdTokenVerifier.Builder(transport, jsonFactory)
            .setAudience(
                Collections.singletonList(appProperties.getOauth2().getGoogle().getClientId()))
            .build();

    final GoogleIdToken googleIdToken = verifier.verify(token);
    if (googleIdToken == null) {
      throw new OAuth2AuthenticationProcessingException("Token Invalid",
          ErrorCode.USER_CREATE_FAIL);
    }
