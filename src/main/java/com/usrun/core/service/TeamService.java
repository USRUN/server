...
public class TeamService {
    ...
    public void buildTeamLeaderBoard() {
        ...
        List<TeamStatDTO> teamStat = teamMemberRepository.getTeamStat();
        List<TeamNewMemberDTO> teamNewMember = teamMemberRepository.getNewMemberInWeek();

        teamNewMember.forEach(tnew -> {
            long teamId = tnew.getTeamId();
            teamStat.stream()
                    .filter(tStat -> (tStat.getTeamId() == teamId))
                    .findFirst()
                    .get()
                    .setMemInWeek(tnew.getNumberNewMember());
        });

        cacheClient.setTeamStat(teamStat);
