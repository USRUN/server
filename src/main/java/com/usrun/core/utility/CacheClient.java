...
public class CacheClient {
  ...
  public TeamStatDTO getTeamStat(long teamId) {
    RBucket<String> rBucket = redissonClient
        .getBucket(cacheKeyGenerator.keyTeamStat(teamId));
    if (rBucket.isExists()) {
      return ObjectUtils.fromJsonString(rBucket.get(), TeamStatDTO.class);
    }
    return null;
  }
  ...
  public void setTeamLeaderBoard(List<TeamLeaderBoardDTO> teamLeaderBoard) {
    ...
    rBucket.set(ObjectUtils.toJsonString(teamLeaderBoard), 7, TimeUnit.DAYS);
  }
  ...
  public List<TeamLeaderBoardDTO> getTeamLeaderBoard(long teamId) {
    ...
    if (rBucket.isExists()) {
      return ObjectUtils.fromJsonString(rBucket.get(),
          new TypeReference<List<TeamLeaderBoardDTO>>() {
          });
    }
    return null;
  }
