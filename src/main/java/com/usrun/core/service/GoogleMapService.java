...
public class GoogleMapService {
    ...
    public String getRouteImage(List<List<Location>> dataLocation) throws IOException {
        ...
        for (List<Location> locations : dataLocation) {
        ...
        if (!dataLocation.isEmpty()) {
            markerStart = "color:blue|label:S|" + dataLocation.get(0).get(0).getLatitude() + "," + dataLocation.get(0).get(0)
                    .getLongitude();
            List<Location> start = dataLocation.get(0);
            List<Location> end = dataLocation.get(dataLocation.size()-1);
            markerFinish = "color:blue|label:F|" + end.get(end.size() - 1).getLatitude() + ","
                    + end.get(end.size() - 1).getLongitude();
        }
        String query = "size=720x720";
        for(String path : listPath){
            query += "&path=" + path;
        }
        query += "&markers=" + URLEncoder
