...
public class OrganizationRepositoryImpl implements OrganizationRepository {
    ...
    private MapSqlParameterSource mapOrganization(Organization organization) {
        ...
        map.addValue("avatar", organization.getAvatar());
        map.addValue("website", organization.getWebsite());
        map.addValue("description", organization.getDescription());
        return map;
    }
    ...
    public int insert(Organization organization) {
        ...
        try {
            int resp = namedParameterJdbcTemplate.update(
                    "INSERT INTO organization(id,name,avatar,description, website)"
                    + " VALUES(:id, :name, :avatar, :description, :website)",
                    map
            );
            return resp;
        } catch (Exception ex) {
    ...
    public int update(Organization organization) {
        ...
        try {
            int resp = namedParameterJdbcTemplate.update(
                    "update organization "
                    + "SET name = :name , avatar = :avatar , website = :website , description = :description "
                    + " WHERE id = :id",
                    map
            );
            return resp;
